...
public class Db {
    ...
    public int getOrCreateSensorId(int estacionId, int tipoSensorId) throws SQLException {
        ...
        try (PreparedStatement ps = conn.prepareStatement(insert, Statement.RETURN_GENERATED_KEYS)) {
            ...
            try (ResultSet keys = ps.getGeneratedKeys()) {
                ...
                if (keys.next()) {
                    ...
                    System.out.println("[DB] Sensor creado: estacion_id=" + estacionId +
                            " tipo=" + tipoSensorId +
                            " id=" + newId);
        ...
        throw new SQLException("No se pudo crear/obtener sensor para estacion_id=" +
                estacionId + " tipo=" + tipoSensorId);
    }
    ...
    public void insertarBatch(Map<Integer, Double> tipoToValor,
    ...
    /**
     * Actualiza el porcentaje de batería de la estación.
     */
    public void actualizarBateria(String estacionCodigo, double bateria) throws SQLException {

        int estacionId = getOrCreateEstacionId(estacionCodigo);

        String sql = "UPDATE estaciones SET bateria = ? WHERE id = ?";

        try (PreparedStatement ps = conn.prepareStatement(sql)) {

            ps.setBigDecimal(1, new java.math.BigDecimal(String.valueOf(bateria)));
